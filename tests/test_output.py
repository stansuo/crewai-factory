...
class TestSavePost:
    def test_creates_file(self, sample_persona: Persona, tmp_path: Path) -> None:
        path = save_post("Hello world", sample_persona, tmp_path)
        assert path.exists()
    ...
    def test_file_contains_content(
        ...
        path = save_post("Test content here", sample_persona, tmp_path)
    ...
    def test_file_contains_header(
        ...
        path = save_post("Content", sample_persona, tmp_path)
        text = path.read_text(encoding="utf-8")
    ...
    def test_header_contains_model(
        ...
        path = save_post("Content", sample_persona, tmp_path, model="test-model:1b")
        text = path.read_text(encoding="utf-8")
    ...
    def test_creates_output_dir(self, sample_persona: Persona, tmp_path: Path) -> None:
        ...
        path = save_post("Content", sample_persona, nested)
        assert path.exists()
    ...
    def test_same_time_no_collision(
        ...
        p1 = save_post("A", sample_persona, tmp_path, now=fixed)
        p2 = save_post("B", sample_persona, tmp_path, now=fixed)
    ...
    def test_different_times_different_files(
        ...
        p1 = save_post("A", sample_persona, tmp_path, now=t1)
        p2 = save_post("B", sample_persona, tmp_path, now=t2)
        assert p1.name != p2.name
